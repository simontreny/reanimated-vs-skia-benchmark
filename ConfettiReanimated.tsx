...
const defaultConfig: ConfettiReanimatedConfig = {
...
type ConfettiReanimatedProps = {
  config?: Partial<ConfettiReanimatedConfig>;
};

export const ConfettiReanimated = ({config}: ConfettiReanimatedProps) => {
...
const Confetti = ({
  ...
  config: ConfettiReanimatedConfig;
}) => {
  ...
  const style = useAnimatedStyle(() => {
    ...
    // Ascending phase
    const ascendingTime = Math.max(time.value - delay, 0);
    ...
    // Hovering phase
    const hoveringTime = Math.max(time.value - delay - ascendingDuration, 0);
    ...
    const descendingTime = Math.max(
      time.value - delay - ascendingDuration - hoveringDuration,
      0,
    );
    ...
    const rotate = `${(hoveringTime / 1000) * rotationVelocity}deg`;
    return {
      position: 'absolute',
      transform: [
  ...
  const {width, height} = Image.resolveAssetSource(animationProps.image);

  return (
    <Animated.Image
      style={[style, {width, height, tintColor: animationProps.color}]}
      source={animationProps.image}
    />
  );
};
